...
#map(int, ...) прогоняет каждое значение и делает из строки число
numbers = list(map(int, input("введи числа через пробел: ").split()))

#sum() сама пробегается по списку и складывает всё в кучу
total = sum(numbers)
...
#тут фишка: sum() + генератор
#он на лету перебирает список и берет только то что больше нуля
